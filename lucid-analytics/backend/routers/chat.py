...
async def get_dropi_data(token: str, country: str, start_date: str, end_date: str) -> dict:
    ...
    from routers.dropi import dropi_request, STATUS_MAP
    ...
    if orders_result.get("success"):
        ...
        STATUS_MAP = {1: "pending", 2: "confirmed", 3: "shipped", 4: "delivered", 5: "returned", 6: "cancelled"}
        ...
        for order in orders:
            ...
            status_name = STATUS_MAP.get(order.get("status", 1), "unknown")
...
async def send_message(
    ...
    # Llamar a Claude
    response_text = await call_claude(system, data.message)
