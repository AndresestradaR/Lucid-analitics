...
## IMPORTANTE SOBRE EL WALLET
- El campo "balance" en dropi.wallet es el SALDO ACTUAL disponible para retirar
- Si te preguntan "cuánto tengo en la wallet" o "mi saldo", responde con el balance actual
- Siempre formatea montos grandes: $11,075.79 no 11075.79
...
**Wallet:**
- balance: Saldo ACTUAL disponible (esto es lo que puede retirar)
- total_ganancias: Suma de pagos recibidos por entregas en el período
- total_devoluciones: Suma de cobros por fletes en el período
- utilidad_neta: Ganancias - Devoluciones del período
...
- Entregas pendientes de pago: Órdenes entregadas que Dropi aún no ha pagado
- Devoluciones pendientes de cobro: Devoluciones que Dropi aún no ha descontado
...
- Formatea números: $2,668,576 (con separadores)
...
async def get_dropi_data(token: str, country: str, start_date: str, end_date: str, db_connection=None) -> dict:
    """Obtener datos de Dropi con la nueva categorización de estados"""
    from routers.dropi import dropi_request, dropi_login
    
    # Obtener wallet balance ACTUAL desde el login (no del historial)
    wallet_balance = 0
    if db_connection:
        try:
            # Desencriptar password
            password = decrypt_token(db_connection.password_encrypted)
            
            # Hacer login para obtener wallet balance actual
            ...
            if login_result.get("success"):
                ...
                print(f"[CHAT] Wallet balance desde login: {wallet_balance}")
        except Exception as e:
            print(f"[CHAT] Error obteniendo wallet desde login: {e}")
    
    # Si no pudimos obtener del login, intentar del historial
    if wallet_balance == 0:
        ...
        if wallet_result.get("success"):
            ...
            if records:
                ...
                print(f"[CHAT] Wallet balance desde historial: {wallet_balance}")
    
    print(f"[CHAT] Wallet balance final: {wallet_balance}")
    
    # Obtener historial de wallet para ganancias/devoluciones (con filtro de fecha)
    ...
    if wallet_history_result.get("success"):
        ...
        print(f"[CHAT] Procesando {len(records)} registros de wallet entre {start_date} y {end_date}")
    ...
    print(f"[CHAT] Ganancias: {wallet_stats['total_ganancias']} ({wallet_stats['count_ganancias']} entregas)")
    print(f"[CHAT] Devoluciones: {wallet_stats['total_devoluciones']} ({wallet_stats['count_devoluciones']} devs)")
...
async def send_message(
    ...
    elif "octubre" in message_lower:
        start_date = "2025-10-01"
        end_date = today_str
        period_label = "Desde Octubre 2025"
    elif "noviembre" in message_lower:
        start_date = "2025-11-01"
        end_date = today_str
        period_label = "Desde Noviembre 2025"
    elif "todo" in message_lower or "total" in message_lower or "completo" in message_lower:
        start_date = (today - timedelta(days=90)).strftime("%Y-%m-%d")
        end_date = today_str
        period_label = "Últimos 90 días"
    else:
        # Default: últimos 60 días para capturar más datos
        start_date = (today - timedelta(days=60)).strftime("%Y-%m-%d")
        end_date = today_str
        period_label = "Últimos 60 días"
    ...
    if dropi_conn:
        try:
            ...
            dropi_data = await get_dropi_data(
                dropi_token, dropi_conn.country, start_date, end_date, dropi_conn
            )
