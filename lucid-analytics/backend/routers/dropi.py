...
async def get_wallet_history(
    ...
    # Llamar a historywallet - probar sin parámetros primero
    ...
    if not result.get("success"):
        print(f"[WALLET HISTORY] Attempt 1 failed, trying with minimal params...")
        # Intento 2: Con result_number bajo
        result = await dropi_request(
            "GET",
            "/api/historywallet",
            token,
            connection.country,
            params={"result_number": 100, "start": 0}
        )
    ...
    try:
        ...
        print(f"[WALLET HISTORY] Date parsing error: {e}")
        end_dt = datetime.now()
        start_dt = end_dt - timedelta(days=days)
