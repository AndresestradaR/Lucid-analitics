...
async def _fetch_orders_stats(token: str, country: str, start_date: str = None, end_date: str = None, days: int = 7):
    ...
    # Mapeo de estados - Dropi usa diferentes valores
    # Según MCP: 1=pending, 2=confirmed, 3=shipped, 4=delivered, 5=returned, 6=cancelled
    # Pero también puede ser un objeto con 'id' o un string
    STATUS_MAP = {
    ...
    for order in orders:
        ...
        # Obtener status - puede ser int, string, o un objeto
        status_raw = order.get("status")
        if isinstance(status_raw, dict):
            status_id = status_raw.get("id", 1)
        else:
        ...
        # Debug: contar status
        status_debug[str(status_id)] = status_debug.get(str(status_id), 0) + 1
    ...
    print(f"[DROPI DEBUG] Orders in range: {stats['total']}, Delivered: {stats['delivered']}")
...
async def get_dropi_summary(
    ...
    if wallet_result.get("success"):
        ...
        print(f"[DROPI DEBUG] Wallet records: {len(records)}, first: {records[0] if records else 'none'}")
        if records:
            wallet_balance = float(records[0].get("balance", 0))
    else:
        print(f"[DROPI DEBUG] Wallet request failed: {wallet_result.get('error', 'unknown')}")
    
    print(f"[DROPI DEBUG] Wallet balance: {wallet_balance}")
