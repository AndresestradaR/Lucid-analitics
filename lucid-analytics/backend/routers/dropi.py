"""
Router de Dropi - CON CACHE LOCAL
Lee de PostgreSQL en lugar de llamar a la API de Dropi directamente.
La sincronización se hace en background.
"""
...
class DropiConnectionResponse(BaseModel):
    ...
    is_active: bool
    created_at: datetime
    last_orders_sync: Optional[datetime]
    last_wallet_sync: Optional[datetime]
...
async def dropi_login(email: str, password: str, country: str) -> dict:
    """Hacer login en Dropi y obtener token + wallet"""
...
async def get_dropi_status(
    ...
    # Contar órdenes y wallet en cache
...
async def trigger_sync(
    ...
    """Disparar sincronización manual de Dropi"""
    connection = db.query(DropiConnection).filter(
        DropiConnection.user_id == current_user.id,
        DropiConnection.is_active == True
    ).first()
...
async def get_wallet(
    ...
    """Obtener saldo actual de wallet (del último login)"""
    connection = db.query(DropiConnection).filter(
        DropiConnection.user_id == current_user.id,
        DropiConnection.is_active == True
    ).first()
    ...
    # Hacer login fresco para obtener balance actual
...
async def get_orders(
    ...
    """Obtener órdenes desde cache local"""
    connection = db.query(DropiConnection).filter(
        DropiConnection.user_id == current_user.id,
        DropiConnection.is_active == True
    ).first()
