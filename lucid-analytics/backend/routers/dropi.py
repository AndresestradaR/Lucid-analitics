...
def get_dropi_headers(token: str = None):
    """Headers para requests a Dropi - según documentación oficial"""
    headers = {
...
async def dropi_login(email: str, password: str, country: str) -> dict:
    ...
    Según documentación: POST /api/login con email, password, white_brand_id
    """
    ...
    # Payload EXACTO según documentación oficial de Dropi
    payload = {
        "email": email,
        "password": password,
    ...
    print(f"[DROPI DEBUG] Payload: email={email}, white_brand_id={white_brand_id[:20]}...")
