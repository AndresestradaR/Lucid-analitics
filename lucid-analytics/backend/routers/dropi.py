...
async def get_dropi_summary(
    ...
    # Obtener wallet - intentar endpoint de usuario primero
    wallet_balance = 0
    
    # Intentar obtener del perfil de usuario
    user_result = await dropi_request(
        "GET",
        "/api/users/me",
        token,
        connection.country
    ...
    if user_result.get("success"):
        ...
        wallet_balance = float(user_data.get("wallet", 0) or user_data.get("balance", 0) or 0)
        print(f"[DROPI DEBUG] Wallet from user profile: {wallet_balance}")
    
    # Si no hay wallet del usuario, intentar historial
    if wallet_balance == 0:
