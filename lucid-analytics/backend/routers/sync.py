...
def sync_contacts_to_db(
    ...
    Usa upsert para manejar duplicados correctamente.
    ...
    updated = 0
    errors = 0
    seen_ids = set()  # Para evitar duplicados en la misma lista
    
    print(f"[SYNC DB] Procesando {len(contacts)} contactos para user_id={user_id}")
    ...
    for contact in contacts:
        try:
            ...
            if lucidbot_id in seen_ids:
                continue
            seen_ids.add(lucidbot_id)
            ...
            if existing:
                ...
                db.add(new_contact)
                db.flush()  # Flush para detectar errores de duplicado temprano
                created += 1
        
        except Exception as e:
            db.rollback()  # Rollback en caso de error
            print(f"[SYNC DB] Error procesando contacto {lucidbot_id}: {str(e)}")
            errors += 1
    
    try:
        db.commit()
        print(f"[SYNC DB] Commit exitoso: {created} creados, {updated} actualizados, {errors} errores")
    except Exception as e:
        db.rollback()
        print(f"[SYNC DB] Error en commit: {str(e)}")
        return {"created": 0, "updated": 0, "errors": len(contacts), "total_processed": len(contacts)}
...
async def sync_ad_contacts(
    ...
    # Limpiar datos previos de este ad_id para este usuario (por si hay datos corruptos)
    deleted = db.query(LucidbotContact).filter(
        LucidbotContact.user_id == current_user.id,
        LucidbotContact.ad_id == ad_id
    ).delete()
    db.commit()
    print(f"[SYNC ENDPOINT] Limpiados {deleted} registros previos de ad_id={ad_id}")
    ...
    # Guardar en BD
    result = sync_contacts_to_db(db, current_user.id, contacts, ad_id)
