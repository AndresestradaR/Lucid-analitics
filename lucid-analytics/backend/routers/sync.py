...
async def fetch_all_contacts_for_ad(
        ...
        while page <= max_pages:
            try:
                ...
                if response.status_code != 200:
                    logger.error(f"Error API LucidBot página {page}: {response.status_code}")
                    break
                ...
                logger.info(f"[SYNC] ad_id={ad_id} página {page}: {len(data)} contactos")
    ...
    logger.info(f"[SYNC] ad_id={ad_id} TOTAL: {len(all_contacts)} contactos")
    return all_contacts
