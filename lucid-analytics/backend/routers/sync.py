...
async def get_sync_status(
    ...
    contacts_by_date = db.query(
        ...
        func.sum(
            case((LucidbotContact.total_a_pagar > 0, 1), else_=0)
        ).label('ventas')
    ).filter(
        LucidbotContact.user_id == current_user.id
    ).group_by(
...
async def save_contacts_with_details(db: Session, user_id: int, contacts: list, jwt_token: str, page_id: str):
    ...
    created = 0
    errors = 0
    skipped = 0  # Contactos actualizados (ya existían)
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for i, contact in enumerate(contacts):
            try:
                ...
                # Parsear fecha - LucidBot devuelve UTC, convertir a Colombia (UTC-5)
                created_at = None
                created_at_str = contact.get("dt")
                if created_at_str:
                    try:
                        # La fecha viene en UTC, restar 5 horas para Colombia
                        created_at = datetime.strptime(created_at_str, "%Y-%m-%d %H:%M:%S") - timedelta(hours=5)
                    except:
                        pass
                
                # Guardar contacto - usar UPSERT (verificar si existe primero)
                lucidbot_id_int = int(ms_id) if ms_id else 0
                
                existing = db.query(LucidbotContact).filter(
                    LucidbotContact.user_id == user_id,
                    LucidbotContact.lucidbot_id == lucidbot_id_int
                ).first()
                
                if existing:
                    # Actualizar existente
                    existing.user_id = user_id
                    existing.phone = contact.get("ph", "")
                    existing.full_name = contact.get("n", "")
                    existing.ad_id = ad_id
                    existing.contact_created_at = created_at
                    existing.calificacion = calificacion
                    existing.producto = producto
                    existing.total_a_pagar = total_a_pagar
                    existing.raw_data = json.dumps(contact)
                ...
                if (created + skipped) % 50 == 0:
                    db.commit()
                    logger.info(f"[SYNC V2] Procesados {created + skipped}/{len(contacts)} contactos...")
                
            except Exception as e:
                errors += 1
                db.rollback()  # Importante: resetear la sesión después de error
                logger.error(f"[SYNC V2] Error guardando contacto {ms_id}: {str(e)[:100]}")
                continue
    ...
    logger.info(f"[SYNC V2]    - Nuevos contactos: {created}")
    logger.info(f"[SYNC V2]    - Actualizados: {skipped}")
