...
@router.get("/diagnostico/{ad_id}")
async def diagnostico_completo(
    ad_id: str,
    db: Session = Depends(get_db)
):
    """
    DIAGNÓSTICO COMPLETO - Compara resultados de día vs semana
    
    Uso: /api/analytics/diagnostico/120236155688730647
    
    NOTA: Endpoint público temporalmente para diagnóstico.
    Usa el primer usuario con LucidBot conectado.
    """
    
    # Buscar cualquier conexión activa de LucidBot para diagnóstico
    lucidbot_conn = db.query(LucidbotConnection).filter(
        LucidbotConnection.is_active == True
    ).first()
