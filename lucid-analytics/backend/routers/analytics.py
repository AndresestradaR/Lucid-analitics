...
# Colombia es UTC-5
# Las fechas en LucidBot están en UTC
# Cuando el usuario pide "1 de diciembre" (Colombia), en UTC es:
# - Inicio: 1 dic 05:00 UTC
# - Fin: 2 dic 04:59:59 UTC
UTC_OFFSET_HOURS = 5
...
def get_lucidbot_data_from_db(
    ...
    """
    try:
        # Convertir fechas Colombia a UTC
        # Colombia 00:00 = UTC 05:00
        start_dt = datetime.strptime(start_date, "%Y-%m-%d") + timedelta(hours=UTC_OFFSET_HOURS)
        # Colombia 23:59:59 = UTC del día siguiente 04:59:59
        end_dt = datetime.strptime(end_date, "%Y-%m-%d").replace(hour=23, minute=59, second=59) + timedelta(hours=UTC_OFFSET_HOURS)
        
        logger.info(f"[DB QUERY] ad_id={ad_id}, UTC range: {start_dt} → {end_dt}")
