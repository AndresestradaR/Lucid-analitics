...
def parse_lucidbot_date(date_str: str) -> Optional[datetime]:
    ...
    - "2025-12-13 21:21:03" (formato de subscribed_date)
    ...
    # Lista de formatos a intentar (orden de prioridad)
    formats = [
        # Formato de subscribed_date de LucidBot API
        "%Y-%m-%d %H:%M:%S",       # "2025-12-13 21:21:03"
...
def is_date_in_range(date_str: str, start_date: str, end_date: str) -> bool:
    ...
    Args:
        date_str: Fecha del contacto (ej: "2025-12-13 21:21:03" o "13/12/2025 14:30")
...
async def get_lucidbot_contacts_by_ad(
    ...
    CORREGIDO: Usa subscribed_date como fuente principal de fecha
    ...
    async with httpx.AsyncClient() as client:
        ...
        for contact in contacts:
            ...
            # ============================================
            # OBTENER FECHA DEL CONTACTO (CORREGIDO)
            # Prioridad: subscribed_date > created_at > custom_fields
            # ============================================
            contact_date = None
            
            # 1. PRIMERO: subscribed_date (formato: "2025-12-13 21:21:03")
            if contact.get("subscribed_date"):
                contact_date = contact.get("subscribed_date")
            
            # 2. SEGUNDO: created_at 
            elif contact.get("created_at"):
                contact_date = contact.get("created_at")
            
            # 3. TERCERO: Buscar en custom_fields
            elif custom_fields.get("Contact"):
                contact_date = custom_fields.get("Contact")
            elif custom_fields.get("Fecha"):
                contact_date = custom_fields.get("Fecha")
            elif custom_fields.get("fecha"):
                contact_date = custom_fields.get("fecha")
            elif custom_fields.get("Fecha_Creacion"):
                contact_date = custom_fields.get("Fecha_Creacion")
            ...
            # ============================================
            # CLASIFICAR VENTA vs LEAD (CORREGIDO)
            # custom_fields es un DICCIONARIO con nombres como keys
            # ============================================
            ...
            if total_paid:
                try:
                    ...
                    clean_value = str(total_paid).replace(",", "").replace("$", "").replace(" ", "").strip()
                    ...
                    # Si tiene valor pero no es numérico, aún considerarlo venta
                    sales += 1
                    contact_info["is_sale"] = True
                    contact_info["amount"] = 0
            else:
                leads += 1
                contact_info["is_sale"] = False
...
async def get_dashboard(
    ...
    for ad in meta_ads:
        ...
        if lucidbot_token:
            ...
            # Guardar debug info
            if lucid_data.get("_debug"):
                debug = lucid_data["_debug"].copy()
                debug["ad_id"] = ad_id
    ...
    return {
        ...
        # Debug info para verificar conteos
        "_debug": {
            "lucidbot_details": all_debug_info
        }
    }
