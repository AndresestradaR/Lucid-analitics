...
def parse_lucidbot_date(date_str: str) -> Optional[datetime]:
    ...
    # LucidBot puede devolver fechas en varios formatos
    formats = [
...
def is_date_in_range(date_str: str, start_date: str, end_date: str) -> bool:
    ...
    try:
        ...
        # El end_date debe incluir todo el día (23:59:59)
...
async def get_lucidbot_contacts_by_ad(
    ...
    async with httpx.AsyncClient() as client:
        ...
        for contact in contacts:
            # NUEVO: Obtener fecha del contacto
            created_at = contact.get("created_at", "")
            
            # También buscar en custom_fields por si la fecha está ahí
            custom_fields = contact.get("custom_fields", {})
            contact_date = created_at or custom_fields.get("Contact", "") or custom_fields.get("Fecha", "")
            
            # NUEVO: Filtrar por fecha - solo contar si está en el rango
            if not is_date_in_range(contact_date, start_date, end_date):
                filtered_out += 1
