...
async def diagnostico_paginacion(
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        ...
        resultados["sin_paginacion"] = len(data.get("data", []))
        
        # Test 2: Con page=1
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id, "page": 1}
        )
        data = resp.json() if resp.status_code == 200 else {}
        resultados["page_1"] = len(data.get("data", []))
        
        # Test 3: Con page=2
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id, "page": 2}
        )
        data = resp.json() if resp.status_code == 200 else {}
        resultados["page_2"] = len(data.get("data", []))
        
        # Test 4: Con limit=500
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
        ...
        data = resp.json() if resp.status_code == 200 else {}
        resultados["limit_500"] = len(data.get("data", []))
        
        # Test 5: Con per_page=500
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id, "per_page": 500}
        )
        data = resp.json() if resp.status_code == 200 else {}
        resultados["per_page_500"] = len(data.get("data", []))
        
        # Test 6: Con offset=100
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id, "offset": 100}
        )
        data = resp.json() if resp.status_code == 200 else {}
        resultados["offset_100"] = len(data.get("data", []))
        
        # Test 7: Con start=100
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id, "start": 100}
        )
        data = resp.json() if resp.status_code == 200 else {}
        resultados["start_100"] = len(data.get("data", []))
        
        # Test 8: Comparar IDs de page=1 vs page=2
        ...
        data1 = resp1.json().get("data", []) if resp1.status_code == 200 else []
        data2 = resp2.json().get("data", []) if resp2.status_code == 200 else []
        
        ids1 = set(c.get("id") for c in data1)
        ids2 = set(c.get("id") for c in data2)
        
        resultados["page1_vs_page2"] = {
            "page1_ids": len(ids1),
            "page2_ids": len(ids2),
            "ids_iguales": ids1 == ids2,
            "ids_en_comun": len(ids1.intersection(ids2))
        }
        
        # Test 9: Ver si hay metadatos de paginación en la respuesta
        resp = await client.get(
            f"{LUCIDBOT_BASE_URL}/users/find_by_custom_field",
            headers={"X-ACCESS-TOKEN": api_token, "Accept": "application/json"},
            params={"field_id": "728462", "value": ad_id}
        )
        full_response = resp.json() if resp.status_code == 200 else {}
        resultados["response_keys"] = list(full_response.keys())
        
        # Buscar metadatos de paginación
        for key in ["total", "total_count", "count", "meta", "pagination", "pages", "next", "has_more"]:
            if key in full_response:
                resultados[f"metadata_{key}"] = full_response[key]
    
    return {
        "ad_id": ad_id,
        "resultados_paginacion": resultados
    }
