...
# Offset para capturar ventas en borde de día
# Solo 1 hora hacia atrás (para casos como 23:45 UTC que son 18:45 Colombia)
# 5 horas hacia adelante (para cubrir el día completo en Colombia)
TIMEZONE_OFFSET_BACK = 1
TIMEZONE_OFFSET_FORWARD = 5
...
def is_date_in_range(date_str: str, start_date: str, end_date: str) -> bool:
    ...
    try:
        # Inicio del día - RESTAR 1 hora para capturar bordes (ej: 23:45 del día anterior)
        start = datetime.strptime(start_date, "%Y-%m-%d") - timedelta(hours=TIMEZONE_OFFSET_BACK)
        
        # Fin del día (23:59:59) + SUMAR 5 horas para compensar UTC vs Colombia
        end = datetime.strptime(end_date, "%Y-%m-%d").replace(hour=23, minute=59, second=59) + timedelta(hours=TIMEZONE_OFFSET_FORWARD)
    ...
    async with httpx.AsyncClient() as client:
        ...
        return {
            ...
            "_debug": {
                ...
                "timezone_offset_back": TIMEZONE_OFFSET_BACK,
                "timezone_offset_forward": TIMEZONE_OFFSET_FORWARD
            }
        }
