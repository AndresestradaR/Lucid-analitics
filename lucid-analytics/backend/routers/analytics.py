...
async def get_lucidbot_contacts_by_ad(
    ...
    for contact in contacts:
        ...
        # 1. PRIMERO buscar en custom_fields "Contact" (es la fecha correcta en LucidBot)
        if custom_fields.get("Contact"):
            contact_date = custom_fields.get("Contact")
        
        # 2. Si no existe Contact, usar created_at
...
@router.get("/diagnostico/fechas/{ad_id}")
async def diagnostico_fechas(
    ad_id: str,
    user_id: int,
    ...
    """
    Analiza TODOS los campos de fecha de los contactos para identificar 
    cuál campo usa LucidBot para filtrar
    """
    lucidbot_conn = db.query(LucidbotConnection).filter(
    ...
    # Analizar todos los campos de fecha posibles
    campos_fecha = {}
    contactos_con_venta = []
    ...
    for contact in contacts:
        ...
        # Solo analizar contactos con venta (Total a pagar)
        total_pagar = custom_fields.get("Total a pagar", "")
        if not total_pagar:
            continue
        ...
        # Este es un contacto con venta - analizar sus fechas
        fechas_encontradas = {
            "contact_id": contact.get("id"),
            "nombre": f"{contact.get('first_name', '')} {contact.get('last_name', '')}".strip(),
            "total_pagar": total_pagar,
            "fechas": {}
        }
        
        # 1. created_at del contacto
        if contact.get("created_at"):
            fechas_encontradas["fechas"]["created_at"] = contact.get("created_at")
        
        # 2. subscribed_date
        if contact.get("subscribed_date"):
            fechas_encontradas["fechas"]["subscribed_date"] = contact.get("subscribed_date")
        
        # 3. last_interaction (timestamp)
        if contact.get("last_interaction"):
            fechas_encontradas["fechas"]["last_interaction"] = contact.get("last_interaction")
        
        # 4. Buscar TODOS los custom fields que parezcan fechas
        for nombre, valor in custom_fields.items():
            if not valor:
                continue
            # Detectar si parece fecha
            valor_str = str(valor)
            if any(x in valor_str for x in ["/", "-"]) and any(c.isdigit() for c in valor_str):
                # Podría ser fecha
                if len(valor_str) >= 8:  # Al menos DD/MM/YY
                    fechas_encontradas["fechas"][f"cf_{nombre}"] = valor_str
        
        contactos_con_venta.append(fechas_encontradas)
        
        # Contar cuántas fechas caen en el rango por cada campo
        for campo, valor in fechas_encontradas["fechas"].items():
            if campo not in campos_fecha:
                campos_fecha[campo] = {"en_rango": 0, "fuera_rango": 0, "ejemplos_en_rango": [], "ejemplos_fuera": []}
            
            # Verificar si está en rango
            en_rango = False
            try:
                fecha = parse_lucidbot_date(str(valor))
                if fecha:
                    fecha_solo = fecha.date()
                    start = datetime.strptime(start_date, "%Y-%m-%d").date()
                    end = datetime.strptime(end_date, "%Y-%m-%d").date()
                    en_rango = start <= fecha_solo <= end
            except:
                pass
            
            if en_rango:
                campos_fecha[campo]["en_rango"] += 1
                if len(campos_fecha[campo]["ejemplos_en_rango"]) < 3:
                    campos_fecha[campo]["ejemplos_en_rango"].append({
                        "nombre": fechas_encontradas["nombre"],
                        "valor": valor
                    })
            else:
                campos_fecha[campo]["fuera_rango"] += 1
                if len(campos_fecha[campo]["ejemplos_fuera"]) < 3:
                    campos_fecha[campo]["ejemplos_fuera"].append({
                        "nombre": fechas_encontradas["nombre"],
                        "valor": valor
                    })
    
    # Ordenar campos por cantidad en rango
    campos_ordenados = sorted(
        campos_fecha.items(), 
        key=lambda x: x[1]["en_rango"], 
        reverse=True
    )
    ...
    return {
        ...
        "contactos_con_venta": len(contactos_con_venta),
        "campos_fecha_analizados": dict(campos_ordenados),
        "mejor_campo_para_filtrar": campos_ordenados[0][0] if campos_ordenados else None,
        "detalle_ventas": contactos_con_venta[:10]  # Primeras 10 ventas con sus fechas
    }
