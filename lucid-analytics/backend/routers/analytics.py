...
def parse_lucidbot_date(date_str: str) -> Optional[datetime]:
    ...
    - "01/12/2025 12:52" (formato Contact)
    - "13/12/2025"
    """
    if not date_str:
        return None
    ...
    # Formatos ISO y estándar primero
    formats = [
        "%Y-%m-%d %H:%M:%S",
        "%Y-%m-%d %H:%M:%S.%f",
        "%Y-%m-%dT%H:%M:%S.%fZ",
...
def is_date_in_range(date_str: str, start_date: str, end_date: str) -> bool:
    ...
    SIN compensación UTC - el campo Contact ya está en hora Colombia.
    Comparamos solo las fechas (YYYY-MM-DD).
    """
...
async def get_lucidbot_contacts_by_ad(
    ...
    USA EL CAMPO "Contact" de custom_fields - NO created_at.
    Contact tiene la fecha real en hora Colombia (ej: 01/12/2025 12:52)
    """
    async with httpx.AsyncClient() as client:
        ...
        for contact in contacts:
            # Convertir custom_fields de lista a diccionario
            custom_fields = {}
            ...
            # USAR CAMPO "Contact" PARA LA FECHA (es la fecha real que ve el usuario)
            contact_date = custom_fields.get("Contact", "")
            
            # Si no hay Contact, intentar otras alternativas
            if not contact_date:
                contact_date = custom_fields.get("Fecha", "") or custom_fields.get("fecha", "")
            
            # Si aún no hay fecha, saltar
            if not contact_date:
                continue
            
            # FILTRAR POR FECHA - sin compensación UTC
