...
async def fetch_lucidbot_contacts(api_token: str, ad_id: str, ad_field_id: str = "728462") -> list:
    ...
    Limitado a 5 páginas (500 contactos) para mantener velocidad.
    """
    all_contacts = []
    page = 1
    max_pages = 5  # Límite para evitar timeouts (500 contactos máx por anuncio)
    ...
    print(f"[LUCIDBOT] ad_id={ad_id} TOTAL={len(all_contacts)} (max {max_pages} pages)")
    return all_contacts
...
async def auto_sync_for_ads(db: Session, user_id: int, ad_ids: list, lucidbot_token: str) -> dict:
    """
    Sincronización automática PARALELA para una lista de anuncios.
    """
    import asyncio
    
    total_stats = {"synced": 0, "new": 0, "updated": 0, "errors": 0}
    
    # Fetch todos los contactos en paralelo
    async def fetch_one(ad_id):
        try:
            contacts = await fetch_lucidbot_contacts(lucidbot_token, ad_id)
            return (ad_id, contacts, None)
        except Exception as e:
            return (ad_id, [], str(e))
    
    results = await asyncio.gather(*[fetch_one(ad_id) for ad_id in ad_ids])
    
    # Procesar resultados y guardar en DB
    for ad_id, contacts, error in results:
        if error:
            print(f"[AUTO-SYNC] Error ad {ad_id}: {error}")
        ...
        if contacts:
            try:
                ...
                print(f"[AUTO-SYNC] DB error ad {ad_id}: {e}")
                db.rollback()
                total_stats["errors"] += 1
