...
async def fetch_lucidbot_contacts(api_token: str, ad_id: str) -> list:
    """Obtener contactos de LucidBot para un anuncio específico."""
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            # Intentar con el token como parámetro (método más común en LucidBot)
            response = await client.get(
                f"{LUCIDBOT_BASE_URL}/contacts",
                params={
                    "ad_id": ad_id,
                    "api_token": api_token  # Token como query param
                }
            )
            print(f"[LUCIDBOT] ad_id={ad_id} method=query_param status={response.status_code}")
            
            # Si falla, intentar con header Authorization sin Bearer
            if response.status_code == 401:
                response = await client.get(
                    f"{LUCIDBOT_BASE_URL}/contacts",
                    params={"ad_id": ad_id},
                    headers={"Authorization": api_token}  # Sin "Bearer"
                )
                print(f"[LUCIDBOT] ad_id={ad_id} method=auth_header status={response.status_code}")
            
            # Si sigue fallando, intentar con X-API-Key
            if response.status_code == 401:
                response = await client.get(
                    f"{LUCIDBOT_BASE_URL}/contacts",
                    params={"ad_id": ad_id},
                ...
                print(f"[LUCIDBOT] ad_id={ad_id} method=x_api_key status={response.status_code}")
            
            if response.status_code == 200:
                data = response.json()
                contacts = data.get("data", []) if isinstance(data, dict) else data
                print(f"[LUCIDBOT] ad_id={ad_id} contacts_count={len(contacts) if contacts else 0}")
                return contacts
            else:
                print(f"[LUCIDBOT] ad_id={ad_id} FAILED all methods. Last response={response.text[:200]}")
            return []
        except Exception as e:
            print(f"[LUCIDBOT] Error fetching ad {ad_id}: {e}")
            return []
...
def sync_contacts_to_db(db: Session, user_id: int, contacts: list, ad_id: str) -> dict:
    ...
    for contact in contacts:
        try:
            ...
            try:
                if created_at_str:
                    if "T" in created_at_str:
                        contact_created_at = datetime.fromisoformat(created_at_str.replace("Z", "+00:00").replace("+00:00", ""))
                    else:
                        contact_created_at = datetime.strptime(created_at_str, "%Y-%m-%d %H:%M:%S")
                else:
            ...
            total_a_pagar = None
            total_str = contact.get("total_a_pagar") or contact.get("total")
            if total_str:
                try:
            ...
            if existing:
                ...
                new_contact = LucidbotContact(
                    ...
                    phone=contact.get("phone") or contact.get("telefono") or "",
                    ad_id=ad_id,
                    total_a_pagar=total_a_pagar,
                    producto=contact.get("producto") or contact.get("product") or "",
                    calificacion=contact.get("calificacion") or contact.get("qualification") or "",
