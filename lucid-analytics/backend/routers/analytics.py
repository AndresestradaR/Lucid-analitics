...
async def fetch_all_contacts_paginated(api_token: str, ad_id: str, ad_field_id: str = "728462") -> list:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        while page <= max_pages:
            try:
                ...
                print(f"[LUCIDBOT] ad={ad_id} page={page} status={response.status_code}")
                
                if response.status_code != 200:
                    print(f"[LUCIDBOT] ad={ad_id} ERROR: {response.text[:200]}")
                    break
                ...
                print(f"[LUCIDBOT] ad={ad_id} page={page} got={len(contacts)} total={len(all_contacts)}")
    ...
    print(f"[LUCIDBOT] ad={ad_id} FINAL total={len(all_contacts)}")
    return all_contacts
...
async def get_lucidbot_contacts_by_ad(api_token: str, ad_id: str, start_date: str, end_date: str, ad_field_id: str = "728462"):
    ...
    print(f"[LUCIDBOT] ad={ad_id} RESULT: total={total_contacts} no_date={no_date} filtered={filtered_out} in_range={len(contact_details)} has_total={has_total_pagar} sales={sales} leads={leads}")
...
async def get_dashboard(
    ...
    print(f"[DASHBOARD] User {current_user.id} - LucidBot connected: {lucidbot_token is not None}")
