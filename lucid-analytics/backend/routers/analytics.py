...
def is_date_in_range(date_str: str, start_date: str, end_date: str) -> bool:
    ...
    IMPORTANTE: La API de LucidBot devuelve created_at en UTC, pero el usuario
    consulta en hora Colombia (UTC-5). Extendemos el rango 5 horas en cada 
    dirección para capturar ventas en los bordes del día.
    ...
    try:
        # Usuario consulta en hora Colombia, API devuelve en UTC
        # Colombia = UTC-5, entonces Colombia 00:00 = UTC 05:00
        # 
        # Para 13/dic Colombia:
        # - Inicio: 13/dic 00:00 Colombia = 13/dic 05:00 UTC
        # - Fin: 13/dic 23:59 Colombia = 14/dic 04:59 UTC
...
async def get_lucidbot_contacts_by_ad(
    ...
    NOTA: El filtro de fecha compensa automáticamente la diferencia 
    de timezone entre UTC (API) y Colombia (usuario).
    ...
    """
    print(f"[LUCIDBOT] Buscando contactos para ad_id={ad_id}, fechas={start_date} a {end_date}")
    
    # PAGINACIÓN: Obtener todos los contactos (máx 10 páginas = 1000 contactos)
    all_contacts = []
    page = 1
    max_pages = 10
    ...
    async with httpx.AsyncClient() as client:
        while page <= max_pages:
            ...
            if response.status_code != 200:
                print(f"[LUCIDBOT] ad={ad_id} page={page} ERROR status={response.status_code}")
                break
            
            page_contacts = response.json().get("data", [])
            print(f"[LUCIDBOT] ad={ad_id} page={page} got={len(page_contacts)} contacts")
    ...
    print(f"[LUCIDBOT] ad={ad_id} TOTAL={len(all_contacts)} contactos (en {page} páginas)")
    ...
    print(f"[LUCIDBOT] ad={ad_id} RESULTADO: leads={leads}, sales={sales}, revenue={revenue} (filtrados={filtered_out}/{total_contacts})")
...
async def get_dashboard(
    ...
    print(f"[DASHBOARD] account_id={account_id}, dates={start_date} to {end_date}")
    print(f"[DASHBOARD] LucidBot connected: {bool(lucidbot_token)}")
    ...
    print(f"[DASHBOARD] Meta ads found: {len(meta_ads) if meta_ads else 0}")
    ...
    for ad in meta_ads:
        ...
        if lucidbot_token:
            ...
            print(f"[DASHBOARD] No LucidBot token - skipping ad {ad_id}")
