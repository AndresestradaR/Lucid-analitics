...
async def fetch_lucidbot_contacts(api_token: str, ad_id: str, ad_field_id: str = "728462") -> list:
    ...
    Usa el endpoint correcto: /users/find_by_custom_field con X-ACCESS-TOKEN
    """
    async with httpx.AsyncClient(timeout=60) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[LUCIDBOT] ad_id={ad_id} status=200 contacts={len(contacts)}")
                return contacts
            else:
                print(f"[LUCIDBOT] ad_id={ad_id} status={response.status_code} error={response.text[:100]}")
                return []
